...
PROCEDURE CONVERT_CONTENT_TABLE
	LOCAL fieldCount
	USE contentTable NEW
	fieldCount := FCount()
	CLOSE contentTable
	IF fieldCount == 7
		RENAME "contentTable.dbf" TO "contentTableOld.dbf"
		CREATE_CONTENU_TABLE()
		USE contentTableOld ALIAS contentOld NEW
		USE contentTable ALIAS content NEW 
			...
			DO WHILE .NOT. EOF()
				...
				APPEND BLANK
				SELECT contentOld
				REPLACE content->DATE WITH contentOld->DATE, ;
						content->NUM_TABLE WITH contentOld->NUM_TABLE, ;
						content->NFACT WITH contentOld->NFACT, ;
						content->CODE_PLAT WITH contentOld->CODE_PLAT, ;
						content->QUANTITE WITH contentOld->QUANTITE, ;
						content->OFFERT WITH contentOld->OFFERT, ;
						content->REMISE WITH contentOld->REMISE
				SKIP
			ENDDO
		CLOSE content
		CLOSE contentOld
