...
PROCEDURE INSERER_BILAN(date, nbClient, total, st_55, st_196, service, emporte, cb, chq, esp, tr)
   ...
   REPLACE bilany->date WITH date, ;
         ...
         bilany->tva_1 WITH COMPUTE_TVA(bilany->st_55, 5.5), ;
         bilany->tva_2 WITH COMPUTE_TVA(bilany->st_196, 19.6), ;
...
PROCEDURE DISP_RECAP_BILAN(annee, mois)
   ...
   LOCAL tva1 := COMPUTE_TVA(GET_TOTAL_FIELD_BILAN(annee, mois, ST_55), 5.5)
   LOCAL tva2 := COMPUTE_TVA(GEt_TOTAL_FIELD_BILAN(annee, mois, ST_196), 19.6)
...
FUNCTION GET_LISTE_BILAN_IMPRESSION(annee, mois)
     ...
     DO WHILE Found()
      AAdd(ret, {PadL(AllTrim(str(Day(bilany->date))), 2, "0")+ "/"+ PadL(AllTrim(str(Month(bilany->date))), 2, "0"),;
				 ...
				 PadL(AllTrim(str(COMPUTE_TVA(bilany->st_55, 5.5))), 10),;
                 PadL(AllTrim(str(COMPUTE_TVA(bilany->st_196, 19.6))), 10),;
